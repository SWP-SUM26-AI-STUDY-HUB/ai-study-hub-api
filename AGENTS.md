...
| `controller/request/StudyMaterialRequest.java` | `{documentId, count, focus}` — generation is always document-scoped (`documentId` required); `count` clamped by RAG (quiz 5–20 / flashcard 5–30); optional `focus` topic, injection-guarded in RAG. |
| `controller/response/QuizGenerateResponse.java` / `FlashcardGenerateResponse.java` | `{quiz/flashcards[], remainingRequests, dailyLimit}` — mirrors `ChatResponse`'s quota fields so the frontend updates the AI-quota badge. |
...
- **Quiz/flashcard generation counts against the daily AI quota** (`AiQuotaService.checkAndIncrement` runs before the RAG call), and a refusal (document too short / fragmented → empty items) **still consumes the quota** — `StudyMaterialServiceImpl` increments first, then calls RAG, mirroring the chat flow. The frontend detects a refusal via an empty `quiz`/`flashcards` list; the RAG reason is surfaced in the `ApiResponse.message`. There is no persistence — each call generates fresh (stateless).
