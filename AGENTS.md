...
**AI Study Hub API** — a Spring Boot 4.0.6 / Java 21 (LTS, virtual threads enabled) backend for a smart study-document platform: upload/store documents, chat with an AI study assistant over document content (RAG), **generate quizzes & flashcards from a document (Gemini, structured JSON)**, manage storage plans + VNPay billing, auto-moderate public documents via the OpenAI Moderation API, let users bookmark/save documents, surface in-app notifications (document lifecycle, reviews/reports, plan expiry, account sanctions), handle reviews/reports, and **feed an admin AI-observability dashboard from the Langfuse Metrics API v2** (latency/cost/token/citation/route metrics — read-only, cached, fail-open). It is a stateless JSON REST API (port `8080`) backed by PostgreSQL 16 + pgvector, Redis 7, AWS S3, and an **external FastAPI RAG microservice**.
...
- **`StudyMaterialClient` → `StudyMaterialClientImpl`**: sibling of `ChatbotClient` — synchronous blocking `WebClient` POST to `${fastapi.rag-quiz-url}` / `${fastapi.rag-flashcard-url}` (`.timeout(study-material-timeout-seconds:60).block()`). Returns structured quiz/flashcard items + a `refused` flag (doc too short/fragmented → empty items). Reuses the shared `WebClient` + `ObjectMapper` beans. Called by `StudyMaterialServiceImpl` after quota + document-access checks.
...
- **JSONB**: `ChatMessageEntity.citations` AND `ChatMessageEntity.materialPayload` are both `@JdbcTypeCode(SqlTypes.JSON) @Column(columnDefinition = "jsonb")` typed as `String`. `materialPayload` (shape `{"type":"QUIZ"|"FLASHCARD","items":[...]}`) is set only on study-material bot messages so quizzes/flashcards replay as cards in chat history.
...
**Caching.** Spring's cache abstraction backs **read-heavy, slowly-changing reads** with Redis via `@Cacheable`/`@CacheEvict` (`config/CacheConfig` → `RedisCacheManager`, per-cache TTL under `app.cache.*`, JSON values via a customized `GenericJackson2JsonRedisSerializer` that registers `JavaTimeModule` for proper `LocalDateTime` serialization and enables default typing for polymorphic deserialization). This is distinct from the ephemeral `StringRedisTemplate` usage (tokens/quota/blacklist). Three caches: `trendingDocuments` (key `page:size`, TTL 10m), `publicTags` (TTL 30m), and `aiMetrics` (key `fromTs:toTs`, TTL 5m — the admin AI/RAG dashboard payload assembled by `AiMetricsServiceImpl` from the Langfuse Metrics API v2; fail-open). **`@Cacheable` is proxy-based → silently no-ops on self-invocation**, so the trending cacheable read lives in its own bean `TrendingDocumentCacheLoader` and its cached payload holds only PUBLIC tags (the user-agnostic view); `TrendingDocumentServiceImpl` then enriches the current owner's PRIVATE tags per request via one cheap `findOwnedDocumentsWithTags` query (no-op for guests/non-owners) — this split keeps the cache globally shareable (one entry per page) without leaking private tags across viewers. `PageImpl` is not JSON-friendly, so the cached value is the flat `TrendingPage` holder DTO. **Any DTO returned by a `@Cacheable` method (`TrendingPage`, `AiMetricsResponse`) MUST carry `@NoArgsConstructor @AllArgsConstructor` alongside `@Data @Builder`** — `@Builder` alone generates only the all-args constructor, so Jackson's default-typing deserializer cannot instantiate it on a cache hit and 500s with `no Creators, like default constructor, exist` (this exact bug was hit by `AiMetricsResponse`; it is NOT caught by the pure-Mockito suite since `@Cacheable` is inert without a Spring context — only an E2E Redis round-trip reproduces it). Eviction: `createPublicTag` clears both caches; `approveDocument`/`rejectDocument`/`deleteDocument` clear `trendingDocuments` (`allEntries=true`, since ranking shifts across all pages); new reviews rely on TTL self-heal (no eviction coupling). Cache annotations are **inert in pure-Mockito unit tests** (no proxy/context), so they're verified structurally, not via a Redis round-trip.
...
**External HTTP.** Use the shared `WebClient` bean for the FastAPI RAG service. (Google OAuth token exchange in `AuthServiceImpl` uses `RestClient` instead — a pre-existing inconsistency.) The **Langfuse Metrics API v2** (`LangfuseMetricsClient`) also reuses the shared `WebClient` bean for the admin AI dashboard (Basic auth, fail-open).
...
**Document soft-delete & restore (lazy RAG purge).** Soft-delete (`deleteDocument` owner path, or `resolveReport` admin path) flips `status → DELETED` + `deletedAt` + `link_share → null` + subtracts storage, and records the prior state in `status_before_deletion` + `deleted_by_admin` — it **no longer touches RAG** (`document_chunks`/parent docs/BM25 survive the retention window so restore is free and instant). RAG is purged only at **hard-delete** (`hardDeleteDocument`, best-effort `ragClient.deleteVectors` first). Owners restore their own soft-deletes via `POST /api/v1/documents/{id}/restore` (`restoreDocument`): flips status back to `status_before_deletion`, restores storage (may set the user `OVERLIMITSTORAGE`), mints a fresh `link_share` only for PUBLIC+COMPLETED, evicts the trending cache, writes a `DOCUMENT_RESTORED` notification — and never calls RAG. Admin-deleted docs (`deleted_by_admin=true`) and `UPLOADING`/null pre-status cannot be restored. `deleteFastApiVectorsAsync` stays `@Async` but is now used by `rejectDocument` only.
...
| `config/CacheConfig.java` | `@EnableCaching` + `RedisCacheManager` (per-cache TTL, custom JSON serializer supporting Java 8 date/time like LocalDateTime). Cache-name constants: `trendingDocuments`, `publicTags`, `aiMetrics`. |
...
| `service/impl/ChatServiceImpl.java` | Chat sessions, AI-quota enforcement, multi-turn `history` construction (last 10 `chat_messages` → RAG), citation extraction from RAG JSON. `toMessageResponse` also parses `material_payload` JSONB → `ChatMessageResponse.{materialType,quiz,flashcards}` so the frontend can render persisted study-material cards. |
| `controller/StudyMaterialController.java` | `/api/v1/study-materials/{quiz,flashcard}` (authenticated): generate quiz/flashcards from one document. Thin — pulls `userId` from the principal, delegates to `StudyMaterialService`, returns `ApiResponse<QuizGenerateResponse>` / `<FlashcardGenerateResponse>`. A refusal (empty items) maps the RAG reason into the `message`. |
| `service/StudyMaterialService.java` + `service/impl/StudyMaterialServiceImpl.java` | Enforces the daily AI quota (`AiQuotaService.checkAndIncrement`, shared counter with chat), validates document access (owner OR public+completed, never deleted — duplicated from `ChatServiceImpl.loadAccessibleDocument`), then delegates to `StudyMaterialClient`. **Persists into a chat session** (`@Transactional`): resolves/creates a `ChatSessionEntity` (title `Quiz·<docTitle>` / `Flashcards·<docTitle>`; reuses `request.sessionId` if the caller passes one), attaches the document, and writes a USER message (the generation request) + a BOT message whose `material_payload` JSONB holds the items (on refusal: reason becomes `content`, no payload). Returns `Outcome<T>{body, message}` — `body` carries the items + `sessionId`; `message` is null on success, the refusal reason otherwise. |
| `service/StudyMaterialClient.java` + `service/impl/StudyMaterialClientImpl.java` | Blocking `WebClient` client for RAG `/quiz/generate` + `/flashcard/generate` (60s timeout). Parses the RAG envelope via `JsonNode` and maps snake_case (`correct_index`) → camelCase DTOs. Sibling of `ChatbotClientImpl` (same shared `WebClient` + `ObjectMapper` beans). |
| `controller/request/StudyMaterialRequest.java` | `{documentId, count, focus, sessionId}` — generation is always document-scoped (`documentId` required); `count` clamped by RAG (quiz 5–20 / flashcard 5–30); optional `focus` topic, injection-guarded in RAG; optional `sessionId` to append to an existing chat session (else a new one is created so the result shows in history). |
| `controller/response/QuizGenerateResponse.java` / `FlashcardGenerateResponse.java` | `{quiz/flashcards[], remainingRequests, dailyLimit, sessionId}` — mirrors `ChatResponse`'s quota fields so the frontend updates the AI-quota badge; `sessionId` is the chat session the generation was persisted to. `ChatMessageResponse` adds `{materialType, quiz, flashcards}` for history replay. |
| `controller/AdminStatsController.java` | `/api/v1/admin/dashboard`: `GET /stats` (users/docs/storage/revenue/signup aggregation) **and `GET /ai-metrics`** (AI/RAG observability from Langfuse). Both ADMIN-only via the `/api/v1/admin/**` rule. `/ai-metrics` defaults to the last 7 days (UTC, truncated to the minute for cache reuse), delegates to `AiMetricsService`, and returns `ApiResponse<AiMetricsResponse>`. |
| `service/AiMetricsService.java` + `service/impl/AiMetricsServiceImpl.java` | Aggregates the admin AI dashboard from the **Langfuse Metrics API v2** (`docs/langfuse-metrics-cookbook.md` §3.1–3.10): latency funnel, request volume, endpoint latency, tokens/cost by model, daily token series, citation coverage, refusals, chat-route distribution, empty-retrieval rate. Fans out ~15 queries in **parallel on a dedicated virtual-thread executor** (`Executors.newVirtualThreadPerTaskExecutor`, drained `@PreDestroy`), maps `JsonNode` → `MetricRow`/`TimeSeriesPoint`, and assembles one `AiMetricsResponse`. `@Cacheable("aiMetrics")` (5m, key `fromTs:toTs`); **fail-open per widget** (any error → empty/zero, never 5xx); unconfigured keys → all-empty payload immediately. |
| `service/impl/LangfuseMetricsClient.java` | Thin blocking `WebClient` GET client for `GET {langfuse.base-url}/api/public/v2/metrics?query={URL-encoded JSON}` with **Basic auth** (`base64(publicKey:secretKey)`, computed in `@PostConstruct`). Returns the response `data` array; **fail-open** (empty array on unconfigured / transport error / timeout / malformed body). Uses `java.net.URLEncoder` to form-encode the payload (NOT `UriComponentsBuilder` — which misreads JSON `{`/`}` as URI-template variables). Sibling of `DocumentRagClient`/`StudyMaterialClientImpl` (same shared `WebClient` + `ObjectMapper` beans). |
| `controller/response/AiMetricsResponse.java` (+ `MetricRow`, `TimeSeriesPoint`) | Aggregate AI-dashboard DTO: per-widget `List<MetricRow>` (label+value) + `tokenTimeSeries` + scalar `citationCoverageAvg`/`emptyRetrievalCount` + derived summary cards (`totalCost`/`totalTokens`/`totalRequests`) + `configured`/`dataAvailable` flags. `empty(from,to,configured)` factory for the fail-open path. |
...
| `service/impl/DocumentPurgeScheduler.java` | Daily 03:00 `@Scheduled`: permanently deletes documents soft-deleted >`app.document.retention-days` ago via `DocumentService.hardDeleteDocument` (per-document transactions). `hardDeleteDocument` now also purges RAG — `ragClient.deleteVectors` runs **first** (best-effort; needs `document_chunks` still present to resolve parent ids) — then S3 (file + preview) + DB dependents + row. Storage was already reconciled at soft-delete time; RAG is intentionally **not** purged at soft-delete (kept for restore — see below). |
...
**Externalized config groups** (`application.yaml`): `aws.s3.*`, `openai.*` (`api-key`, `moderation-url`), `fastapi.*` (`base-url`, `rag-process-url`, `rag-chat-url`, `rag-quiz-url`, `rag-flashcard-url`, `chat-timeout-seconds:30`, `study-material-timeout-seconds:60` — generation is a larger Gemini call than chat), `app.jwt.*`, `app.internal.secret`, `app.async.*`, `app.cache.*` (`trending-documents`/`public-tags`/`default`/`ai-metrics` TTL minutes), `app.moderation.*` (stream-key/group/consumer-name/poll/batch/max-attempts/dlq/reclaim), `app.upload.max-file-size-bytes` (50MB), `app.document.retention-days` (default 30; soft-delete retention before permanent purge), `app.share-url-prefix`, `app.frontend-url` (VNPay callback redirect target, default `http://localhost:5173`), `langfuse.*` (`base-url`, `public-key`, `secret-key`, `timeout-seconds` — admin AI dashboard observability; blank keys ⇒ `AiMetricsService` returns an all-empty fail-open payload), `vnpay.*`, `spring.security.oauth2.client` (Google), `spring.mail.*`, plus `spring.jackson.time-zone: UTC` + `spring.jpa.properties.hibernate.jdbc.time_zone: UTC`.
...
| API → RAG | `POST /api/v1/quiz/generate` | `{document_id, count, focus}` | Generate a quiz (structured JSON). `StudyMaterialClient.generateQuiz` posts to `fastapi.rag-quiz-url` (60s timeout). Response `data.quiz[]` = `{question, options[4], correct_index, explanation}`; a refusal returns `quiz=[]` + `data.debug.refused=true`. |
| API → RAG | `POST /api/v1/flashcard/generate` | `{document_id, count, focus}` | Generate flashcards. `StudyMaterialClient.generateFlashcard` posts to `fastapi.rag-flashcard-url`. Response `data.flashcards[]` = `{term, definition}`; same refusal semantics as quiz. |
...
- **Guest access restrictions**: For public preview `/api/v1/documents/{id}/preview` and shared links `/api/v1/documents/shared/{token}`, guest users (unauthenticated) are strictly restricted to the 30% preview file (`_preview` suffix in S3). Authenticated users receive the full document path.
...
- **RAG endpoints for reference**: `POST /api/v1/rag/process`, `POST /api/v1/rag/extract`, `POST /api/v1/rag/index`, `PATCH /api/v1/rag/documents/{id}/visibility`, `DELETE /api/v1/rag/documents/{id}`, `POST /api/v1/chat`, `POST /api/v1/quiz/generate`, `POST /api/v1/flashcard/generate`. (The old `/api/v1/chat/retrieve` debug endpoint was removed.) Quiz/flashcard generation is document-scoped (no `user_id` — this API gates document access), returns structured JSON, and a refusal returns HTTP 200 with empty items + `data.debug.refused`.
- **Quiz/flashcard generation counts against the daily AI quota AND is persisted into a chat session** (`AiQuotaService.checkAndIncrement` runs before the RAG call), and a refusal (document too short / fragmented → empty items) **still consumes the quota** — `StudyMaterialServiceImpl` increments first, then calls RAG, mirroring the chat flow. The frontend detects a refusal via an empty `quiz`/`flashcards` list; the RAG reason is surfaced in the `ApiResponse.message`. Persisted as a USER + BOT `chat_messages` pair (bot row carries `material_payload` JSONB), so generations show up in `GET /chat/sessions` + `/messages`. **Schema**: `chat_messages.material_payload jsonb` must exist — added to `initdb.sql`; running DBs need `ALTER TABLE chat_messages ADD COLUMN IF NOT EXISTS material_payload jsonb;` (prod runs `ddl-auto: none`).
