...
*   **Scheduled Jobs:** three daily business jobs — **plan-expiry notification** (08:00), **plan downgrade sweep** (08:00), and **document purge** of soft-deleted docs older than 30 days (03:00); an **AI-metrics cache refresh** (6×/day at 06:00/09:00/12:00/15:00/18:00/21:00, `Asia/Ho_Chi_Minh`) — the sole caller of the Langfuse Metrics API v2 that warms the admin AI-dashboard cache so on-demand `/admin/dashboard/ai-metrics` reads never burst the Hobby-plan 100-requests/day cap; plus a moderation-stream **PEL reclaim** that scans every 60 s and re-claims stream entries idle **> 5 min** (so failed/crashed jobs get retried without stealing in-flight work).
